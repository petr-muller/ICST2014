...
aspects of fuzz testing D-Bus services. We have met developer opinions that the
...
dbusfuzz.py~\footnote{\url{https://chromium.googlesource.com/chromiumos/platform/dbusfuzz/+/master/dbusfuzz.py}},
...
also targets D-Bus services. The comparsion of the main features of these two
...
It mostly seems that dbusfuzz.py is designed to be used programatically from the
outside (as it consumes YAML as input and emits failures as Python tracebacks),
so it is basically a functional subset of \textsc{Dfuzzer}.
...
BendyBus\footnote{\url{http://tecnocode.co.uk/2012/01/19/all-aboard-the-bendy-bus/}} is an another fuzzing tool for fuzz testing applications. Dfuzzer is fuzz
...
Both defects were reported to the GNOME Bugzilla\addtodo{a link?}, and we met
...
was chosen because it is supposed to process inputs in different human
languages.
...
classifications and origin}, some of them severe.

The most critical bug found
...
Fedora GNU/Linux distribution, which use D-Bus for communication. As oppposed to
Red Hat Enterprise Linux distribution focused on servers, Fedora contains many
additional modern desktop applications providing services via D-Bus.
