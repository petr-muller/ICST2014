...
area\cite{takanen08-book,sutton07-book}, because the type of bugs it finds often
has security implications: improper validation sometimes means there is a possibility to carefully craft the input so the
...
The second limitaton, in planning to be removed by further development, is the
...
at the time.
By testing its D-Bus interfaces we immedately found it does not validate its
...
Next selected application was IMSettings is a~framework that handles input
method settings and~immediately propagages a change after it is done, so it will
take effect without~restarting applications and~the~desktop. This application
was chosen because it processes inputs from different languages.
...
distribution components are not installed or running by default).

Even in this limited campaing, we found 16 bugs.\addtodo{approximately half of
...
was a removal of a root account, occuring during fuzz testing a service
